...
When your frontend makes a request to your backend, it [includes an access token](https://docs.propelauth.com/how-tos/making-authenticated-requests). But, what if we don’t have a frontend yet? Or what if we want to test our backend independently from our frontend?
...
There are a few different ways that you can get started with initializing this process, but they all boil down to creating an [Access Token](https://docs.propelauth.com/overview/access-tokens) for use in your testing.
...
```python
╰─ python
Python 3.8.13 (default, May  2 2022, 01:02:44)
...
![Untitled](/images/testing-your-backend-postman-request.png)

Then, if everything is running correctly, it’s returned as if my user made the request

![Untitled](/images/testing-your-backend-postman-response.png)

Our backend libraries are designed to validate access tokens that are sent to them. Typically, the tokens are retrieved from our client libraries, however we can also generate them ourselves so that we can test our APIs independently from the frontend