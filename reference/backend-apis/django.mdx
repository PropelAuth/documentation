...
Our frontend libraries allow you to get [access token](/overview/access-tokens) that can be sent to the backend.
The backend is then responsible for verifying these tokens.

For testing purposes, it can be valuable to be able to create access tokens separately from the frontend,
and set a custom expiration time. This API allows you to do just that - create the same access tokens that the frontend
gets, but you can set the expiration.
