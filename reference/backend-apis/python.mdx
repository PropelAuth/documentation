...
The return value has a `user` and a `org_member_info`
...
`org_member_info` returned by [validate_access_token_and_get_user](#validate-access-token-and-get-user)
and [validate_access_token_and_get_user_with_org](#validate-access-token-and-get-user-with-org).
...
```python
# auth object froms from init_base_auth
