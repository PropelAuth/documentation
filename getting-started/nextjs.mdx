  ...
  <Tab title="Using App Router (Server Components)">
	...
	<Tab title="Using Pages (getServerSideProps)">
		...
		export const getServerSideProps: GetServerSideProps = async (context) => {
			const user = await getUserFromServerSideProps(context, true)
			if (!user) {
...
For the reidrect functions, you can either call them as seen below, or provide alternative paths post login or signup using the following pattern: 
