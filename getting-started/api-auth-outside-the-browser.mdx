...
can then verify that the request is coming from a real user, and receieve any relevant metadata from the requester.

As an example, below is a snippet of a potential **Express** backend that is logging to the console the user metadata for a request after using the **validatePersonalApiKey** endpoint.

```js
app.post('/api/company-names', async (req, res) => {
    const userMetadata = await auth.validatePersonalApiKey(req.headers.authorization)
